...
it('can render register, request register api', () => {
  ...
  const wrapper = shallow(<Form formName="register" post={(payload) => post(payload)} status={null} />);
  ...
  return requestRegister(request).then(res => {
...
it('can render logout, request logout api', () => {
  ...
  return requestLogout(accessToken).then(res => expect(res.response.statusText).toEqual('OK'));
});
...
it('can render login, request login api', () => {
  ...
  const wrapper = shallow(<Form formName="login" post={(payload) => post(payload)} status={null} />);
  wrapper.setState({userId: random});
  wrapper.setState({password: random});
  wrapper.find('button').simulate('click');
  ...
  return requestLogin(request).then(res => expect(res.response.statusText).toEqual('OK'));
});
